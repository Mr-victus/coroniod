...
class OrderIndivisual extends Component {
  ...
  render() {
    return (
      ...
      elevation: 6,}}>
        <View style={{flexDirection:'row',borderRadius:nw(15),backgroundColor:'white',height:nh(350),justifyContent:'space-between',alignItems:'center'}}>
        <View > 
        <View style={{flexDirection:'row',justifyContent:'space-around'}} >
                <Text style={{fontSize:np(15)}}>{this.props.date||"22/03/2020"}</Text>
                <TextInput keyboardType='number-pad' theme={{ colors: {
                    placeholder: '#FF4141', text: 'black', primary: '#FF4141',
                    underlineColor: 'transparent', background: '#003489'
            } }} multiline={true} style={{width:nw(50),height:nh(50),marginBottom:20,backgroundColor:'white',color:'black'}} label='Day' value={this.state.day} onChangeText={(text)=>this.setState({day:text})} />
                <Text style={{fontSize:np(15)}}>{this.props.name||"Jhon Doe"}</Text>
            </View>
            <View > 
            {this.props.images!=undefined?<SliderBox style={{height:nh(220),width:nw(345),backgroundColor:'#EDEDED'}} resizeMode='center'  images={this.props.images||[images.upload]} />:<SliderBox style={{height:nh(220),width:nw(345),backgroundColor:'#EDEDED'}} resizeMode='center'  images={[images.upload]} />}
      ...
      <View style={{marginHorizontal:nw(10)}}>
        ...
        <TextInput theme={{ colors: {
                    placeholder: '#FF4141', text: 'black', primary: '#FF4141',
                    underlineColor: 'transparent', background: '#003489'
            } }} multiline={true} style={{marginHorizontal:50,marginBottom:20,backgroundColor:'white',color:'black'}} label='Caption' value={this.state.caption} onChangeText={(text)=>this.setState({caption:text})} />
